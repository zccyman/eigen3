...
# @Time    : 2022/07/05 15:01:38
# @File    : test_all_layer_export.py
import sys
...
try:
    from utest import MultiLayerBase, parse_config
except:
    from onnx_converter.utest import MultiLayerBase, parse_config # type: ignore

cfg_file = "tests/export_tests/arguments_all_layer.json"
...
@pytest.fixture(
    scope="class", params=quantize_method_process_scale_layer_type_in_type_out_type_list
)
...
class TestLayers(MultiLayerBase): # type: ignore
    def setup_class(self):
        ...
        self.mode = "wb"
        if os.path.exists(self.weights_dir): # type: ignores
    ...
    def test_layer(
        ...
        self.is_export_model_c = True if chip_type in self.chips_only_support_model_c else False
...
if __name__ == "__main__":
    pytest.main(["tests/export_tests/test_all_layer_export.py::TestLayers::test_layer"])
