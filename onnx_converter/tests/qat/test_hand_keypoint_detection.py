...
def get_max_preds(batch_heatmaps):
    ...
    assert batch_heatmaps.dim() == 4, "batch_heatmaps should be 4-dim"
    ...
    _, idx = torch.max(heatmaps_reshaped, 2)
    ...
    preds[:, :, 0] = preds[:, :, 0] % width
    preds[:, :, 1] = torch.floor_divide(preds[:, :, 1], width)

    pred_mask = maxvals.gt(0.0).repeat(1, 1, 2).float()
...
class HandKeyPointPostProcess(object):
    ...
    def __call__(self, outputs, trans):
        ...
        index = index.view(-1)
        kp_point[index, :] = torch.tensor([0, 0], dtype=kp_point.dtype, device=kp_point.device)
        kp_point = torch.abs(kp_point)

        results = torch.cat([kp_point, kp_score], dim=-1)

        return {'heatmap': predicts, 'results': results, 'down_scale': down_scale}
...
if __name__ == "__main__":
    ...
    cocoeval(config_file="tests/qat/test_hand_keypoint_detection.json")