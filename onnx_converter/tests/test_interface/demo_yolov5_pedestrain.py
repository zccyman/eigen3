...
# @Time     : 2023/7/16
# @File     : demo_yolov5_pedestrain.py
...
class Yolov5ISPPreProcess(Yolov5PreProcess):
    ...
    def __call__(self, img):
        ...
        # normalize image
        img_input = img_resize.astype(np.int32) * 514
...
class CocoDataset(object):
    def __init__(self, **kwargs):
        ...
        self.coco_json_path = os.path.join(kwargs["evaluation_dataset_dir"], "cocoperson.json")
...
class YoloEvaluator(object):
    ...
    def __call__(self, converter):
        ...
        for data in tqdm(iter(self.dataset)):
            ...
            pred_cvt = self.postprocess(results_cvt, input_shape=trans["input_shape"], raw_shape=trans["raw_shape"])
            qmydict = dict()
...
def ispdata_export(converter: OnnxConverter, abgr_file: str):
    ...
    pred_cvt = postprocess(results_cvt, input_shape=trans["input_shape"], raw_shape=trans["raw_shape"])
    qres = postprocess.draw_image(copy.deepcopy(dummy_input), pred_cvt, class_names=class_names, colors=_COLORS)
    cv2.imwrite("/home/shiqing/Downloads/test_package/saturation/onnx-converter/test_pedestrain_abgr.jpg", qres)  ### results for draw bbox
    print("******* Finish model convert *******")

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--args_json', default='/home/shiqing/Downloads/test_package/saturation/onnx-converter/tests/test_interface/arguments_pedestrain.json', type=str)
    parser.add_argument('--model_export', type=bool, default=False)
...
if __name__ == "__main__":
    ...
    args_cvt["post_process"] = Yolov5PostProcess()
    # Build onnx converter
    converter = OnnxConverter(**args_cvt)     
    
    converter.load_model("/buffer/trained_models/pedestrian-detection/yolov5n_320_v2_simplify.onnx")
