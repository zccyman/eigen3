...
class Yolov5PreProcess(object):
    def __init__(self, **kwargs):
        ...
        self.trans = 0
        self.input_size = kwargs.get('input_size', [320, 320])
        self.stride = kwargs.get('stride', [8, 16, 32]) # nano [8, 16, 32, 64]
...
class CocoDataset(object):
    def __init__(self, **kwargs):
        ...
        coco_json_path = os.path.join(kwargs["evaluation_dataset_dir"], "cocoperson.json")
        dataset_dir = os.path.join(kwargs["evaluation_dataset_dir"], "images")
        assert os.path.exists(coco_json_path), 'coco json is not found, please check the file exists'
        assert os.path.exists(dataset_dir), 'dataset root path is not found, please check the folder exists'

        self.image_path_dict=dict()
        self.image_path_list=list()
        self.image_array_list, self.annotation_list, self.trans_list = self.parse_dataset(coco_json_path=coco_json_path, dataset_dir=dataset_dir)
    ...
    def parse_dataset(self, **kwargs):
        self.ann_path = kwargs['coco_json_path']
        self.image_dir = kwargs['dataset_dir']
        coco_annot = json.load(open(self.ann_path, 'r'))
        image_id_dict = dict()
        image_array_list = list()
        trans_list=list()
        
        for (i,image_info) in enumerate(coco_annot['images']):
            image_id = image_info['id']
            img_path = os.path.join(self.image_dir, image_info['file_name'])
            self.image_path_dict.update({image_id:img_path})
            image = cv2.imread(img_path)
            #image = self.expand_input_dims(image)
            image_array_list.append(image)
            image_id_dict.update({image_id:i})

        annotation_list = [[]] * len(image_array_list)
        for annot in coco_annot['annotations']:
            ann = dict()
            ...
            ann.update({"image_id":image_id, "label":label, "bbox":bbox})
            id = image_id_dict[image_id]
            if annotation_list[id] ==[]:
                annotation_list[id] = [ann]
            else:
                annotation_list[id].append(ann)
        return image_array_list, annotation_list,trans_list
    ...
    def __next__(self):
        ...
        return (self.image_array_list[self.start_num], self.annotation_list[self.start_num], self.trans_list[self.start_num])

class Yolov5PostProcess(object):
    def __init__(self, **kwargs):
        super(Yolov5PostProcess, self).__init__()
        self.strides = kwargs.get("strides", [8, 16, 32])
    ...
    def __call__(self, outputs, input_shape, raw_shape):
        device = "cpu"
        input_size, raw_size = input_shape, raw_shape
        z = list()
...
class YoloEvaluator(object):
    def __init__(self, **kwargs):
        super(YoloEvaluator, self).__init__(**kwargs)        
    ...
    def __call__(self):
        for (img, gt, trans) in tqdm(self.dataset):
            
            results = self.converter.model_simulation(img)
            results_cvt = results['result_converter']
            pred_cvt = self.postprocess(results_cvt, trans)
            self.update_result(gt, pred_cvt)
        self.calclate_final_result()
        self.print_result()

    def update_result(self, ann, pred_cvt):
        image_id = ann[0]['image_id']
        num_qface, num_tface=0,0
        if pred_cvt:
            num_qface = pred_cvt['score'].size
            self.qdet_face += num_qface

        num_gt = len(ann)
        self.count_face += num_gt

        for i in range(num_qface):
            self.qdet_boxes['face'].append(
                dict(confidence = pred_cvt['score'][i],
                    file_id = image_id,
                    bbox = pred_cvt['bbox'][i].tolist()))
        ...
        for gt in ann:
            gts.append(dict(
                ...
                file_id=image_id,
                bbox = gt['bbox']))
        self.gt_boxes[image_id] = gts

        if self.show_image:
            img_path = self.dataset.id_to_image_path(image_id)
            img_show = cv2.imread(img_path)
            qres = self.draw_image(img_show, pred_cvt, colors=[(1, 0, 0)])
            cv2.imwrite(os.path.join(self.save_eval_path, img_path.split('/')[-1]), qres)  

    def compute_tp_fp(self, dt_per_classes_dict, cate, gt_data, MIN_OVERLAP=0.5):
        ...
        conf = np.array(scores)[idx_sort]
        tp2 = list(np.array(tp)[idx_sort])
        fp2 = list(np.array(fp)[idx_sort])
        
        return tp2, fp2

    def get_recall_and_precision(self, det_boxes, label, gt_boxes, count_face):
        gt_boxes_ = copy.deepcopy(gt_boxes)
        det_boxes_ = copy.deepcopy(det_boxes)
        tp, fp = self.compute_tp_fp(det_boxes_, label, gt_boxes_, self.iou_threshold)
        recall = np.sum(tp) / count_face
        precision = np.sum(tp) / len(det_boxes['face'])
        return recall, precision

    def calclate_final_result(self):
        qrecall, qprecision = self.get_recall_and_precision(self.qdet_boxes, "face", self.gt_boxes, self.count_face)
        self.accuracy = dict(qaccuracy=dict(Recall=qrecall, Precision=qprecision))

    def print_result(self):
        cvt_res =self.accuracy['qaccuracy']
        print(" result  | Recall | Precision |\n")
        print("converter| %.6f | %.6f   |\n" % (
        cvt_res['Recall'], cvt_res['Precision']))

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--args_json', default='arguments_pedestrain.json', type=str)
...
if __name__ == "__main__":
    ...
    eavl_dataset = CocoDataset(evaluation_dataset_dir=evaluation_dataset_dir)
    args_cvt["transform"] = eavl_dataset.preprocess
    # Build onnx converter
    converter = OnnxConverter(**args_cvt)     
    
    converter.load_model("/buffer/trained_models/pedestrian-detection/yolov5s_320_v1_simplify.onnx")
    ...
    # Build evaluator
    evaluator = YoloEvaluator(dataset_eval=eavl_dataset)
