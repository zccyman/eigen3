...
try:
    ...
    from simulator import Simulation, ErrorAnalyzer
    from simulator import error_factory
except:
    from onnx_converter.utils import Object, process_im, Similarity
    from onnx_converter.utils import extract_scale
    from onnx_converter.simulator import Simulation, ErrorAnalyzer
...
if __name__ == '__main__':
    ...
    dataset_analyzer = ImageNetDataset(preprocess_args=kwargs_preprocess, dataset_dir=args.dataset_path,image_type="JPEG" )
    quan_graph = myClsEval.process.graph
    onnx_infer = OnnxInfer(model_path=args.model_path, input_names =['input'], output_names=['output'])
    analyzer_args = {
        "dataset":dataset_analyzer,
        'quan_graph':quan_graph,
        'onnx_infer':onnx_infer,
        'output_dir':"work_dir/error_analyzer_test/",
        'simulation_level': None
    }
    error_analyzer = ErrorAnalyzer(**analyzer_args)
    error_analyzer()
