...
class_names_yolo = [
    'fist', 'palm', 'like', 'no_gesture'
]

class_names_nano = [
    'one', 'two_up', 'two_up_inverted', 'three', 'three2', 'four',
    'fist', 'palm', 'ok', 'peace', 'peace_inverted',
    'like', 'dislike', 'stop', 'stop_inverted', 'call', 'mute',
    'rock', 'no_gesture'
]
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # np.save(f"gesture/pre_nms_box.npy", (boxes-c).numpy().astype(np.float32))
...
class Integral(nn.Module):
    ...
    def forward(self, x):
        ...
        #np.save("after_softmax.npy", x.reshape(-1).numpy().astype(np.float32))
        x = F.linear(x, self.project.type_as(x)).reshape(-1, 4)
        #np.save("after_linear.npy", x.reshape(-1).numpy().astype(np.float32))        
        return x


def batched_nms(boxes, scores, idxs, iou_threshold, num_candidate=10000, class_agnostic=False):
    if class_agnostic:
        boxes_for_nms = boxes
    else:
...
def multiclass_nms(multi_bboxes,
    ...
    dets, keep = batched_nms(bboxes, scores, labels, iou_threshold, 10000, class_agnostic)
...
def get_bboxes(cls_preds, reg_preds, img_shape, num_candidate, strides,
    ...
    #np.save("center_priors.npy", center_priors.reshape(-1).numpy().astype(np.float32))
    #np.save("distribute_reg_preds.npy", distribution_project(reg_preds).reshape(-1).numpy().astype(np.float32))
    dis_preds = distribution_project(reg_preds) * center_priors[..., 2, None]
    #np.save("dis_preds.npy", dis_preds.reshape(-1).numpy().astype(np.float32))
    bboxes = distance2bbox(center_priors[..., :2], dis_preds, max_shape=input_shape)
    #np.save("bboxes.npy", bboxes.reshape(-1).numpy().astype(np.float32))
    scores = cls_preds.sigmoid()
    #np.save("scores.npy", scores.reshape(-1).numpy().astype(np.float32))
    result_list = []
    for i in range(b):
...
def post_process(
    ...
    #np.save("cls_scores.npy", cls_scores.reshape(-1).numpy().astype(np.float32))
    #np.save("bbox_preds.npy", bbox_preds.reshape(-1).numpy().astype(np.float32))
    ...
    for result in result_list:
        ...
        #np.save("det_bboxes.npy", det_bboxes[:, :4].reshape(-1).numpy().astype(np.float32))
        #np.save("det_scores.npy", det_bboxes[:, 4:].reshape(-1).numpy().astype(np.float32))
        #np.save("det_labels.npy", det_labels.reshape(-1).numpy().astype(np.float32))
        det_bboxes = det_bboxes.detach().cpu().numpy()
        det_bboxes[:, :4] = warp_boxes(
            det_bboxes[:, :4], np.linalg.inv(warp_matrix), img_width, img_height
        )
        #np.save("det_bboxes_last.npy", det_bboxes[:, :4].reshape(-1).astype(np.float32))
        #np.save("det_scores_last.npy", det_bboxes[:, 4:].reshape(-1).astype(np.float32))        
...
class Yolov5PostProcess(object):
    ...
    def __call__(self, outputs, trans):
        ...
        for i, key in enumerate(outputs.keys()):
            ...
            # np.save(f"gesture/output{i+1}.npy", output.numpy().astype(np.float32))
            output = output.view(bs, 3, 9, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
        ...
        # np.save(f"gesture/post_nms_box.npy", torch.cat(pred, dim=0)[:, :5].numpy().astype(np.float32))
        bbox, score, label = list(), list(), list()
        for i, det in enumerate(pred):
            det_tmp = scale_coords(input_size[:2], det[:, :4], raw_size[:2])
            # np.save(f"gesture/post_scale_coords_box.npy", 
                    # torch.cat([det_tmp, det[:, -2].unsqueeze(dim=0)], dim=1).numpy().astype(np.float32))
            det[:, :4] = det_tmp.round()
            bbox.append(det[:, :4])
            score.append(det[:, -2])
            label.append(det[:, -1])
        ...
        label = np.where(label == 1, 2, np.where(label == 2, 1, label))
...
class NanodetPreProcess(object):
    ...
    def __call__(self, img):
        ...
        # resize image
        ResizeM = self.get_resize_matrix((img.shape[1], img.shape[0]), self.input_size, False)
        #np.save("ResizeM.npy", ResizeM.reshape(-1).astype(np.float32))
        img_resize = cv2.warpPerspective(img, ResizeM, dsize=tuple(self.input_size))
        #np.save("img_resize.npy", img_resize.reshape(-1).astype(np.float32))
        # normalize image
        ...
        img_input = (img_input - img_mean) / img_std
        #np.save("img_input_normalize.npy", img_input.reshape(-1).astype(np.float32))
        # expand dims
        img_input = np.transpose(img_input, [2, 0, 1])
        img_input = np.expand_dims(img_input, axis=0)
        #np.save("img_input.npy", img_input.reshape(-1).astype(np.float32))
        res['ResizeM'] = ResizeM
        res['input_shape'] = self.input_size
        self.set_trans(res)
...
class NanodetPostProcess(object):
    ...
    def __call__(self, outputs, trans):
        ...
        for key in outputs.keys():
            if outputs[key] is None:
                continue
            # np.save(f"{key}.npy", outputs[key].reshape(-1).astype(np.float32))
            single_output = torch.from_numpy(outputs[key])
            single_output = single_output.flatten(start_dim=2).permute(0, 2, 1)
            # np.save(f"{key}_transpose.npy", single_output.reshape(-1).numpy().astype(np.float32))
            res.append(single_output)
        if res == list():
            return None
        preds = torch.cat(res, dim=1)
        # #np.save("preds.npy", preds.reshape(-1).numpy().astype(np.float32))
        # preds, warp_matrix, resizeed_shape, raw_shape, strides,
        # num_candidate, score_threshold, iou_threshold, class_agnostic
        outs = post_process(preds, ResizeM, input_shape, raw_shape, self.strides, self.num_candidate,
                           self.prob_threshold, self.iou_threshold, self.agnostic_nms, 
                           self.num_classes, self.reg_max)
        bboxes = []
        scores = []
        labels = []
        for label in outs.keys():
           for bbox_per_class in outs[label]:
                bbox_per_class = np.array(bbox_per_class)
                bbox = bbox_per_class[:4].astype(np.int32)
                score = np.array(bbox_per_class[-1])
                bboxes.append(bbox)
                scores.append(score)
                labels.append(label)
        ...
        return dict(bbox=bboxes, label=labels, score=scores)
...
class_names = {'nano': class_names_nano, 'yolov5': class_names_yolo}

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--model_path', type=str,
                        # default='gesture_recognition_yolov5n_320_4cls.onnx',
                        # default='/buffer2/zhangcc/trained_models/gesture_recognition/yolov5n_1.0x_320_4cls.onnx',
                        default='nanodet_416_simplify.onnx',
                        # default='new_best.onnx',
                        # default='retina_slim_640_3_delete_no_gesture.onnx',
    ...
    parser.add_argument('--quan_dataset_path', type=str, 
                        ...
                        default='/buffer/calibrate_dataset/gesture_recognition',
                        # default='/buffer/gesture/coco/val',
                        help='quantize image path')
    parser.add_argument('--ann_path', type=str,
                        # default='/buffer/crowdhuman/annotation_val.json',
                        # default='/buffer/gesture/coco/light_hagrid_to_coco_val.json',
                        # default='/buffer/gesture/coco/light_hagrid_to_coco_val_delete_no_gesture.json',
                        # default='/buffer2/zhangcc/dataset/gesture/light_hagrid_to_coco_val_class_num_4.json',
                        default='/buffer2/zhangcc/dataset/gesture/light_hagrid_to_coco_val_class_num_19.json',
                        help='eval anno path')
    parser.add_argument('--dataset_path', type=str,
                        # default='/buffer/crowdhuman/val',
                        default='/buffer2/zhangcc/dataset/gesture/val',
                        # default='/buffer/gesture/coco/val',
                        help='eval image path')
    parser.add_argument('--model_name', type=str, default="nano") # nano | yolov5
    parser.add_argument('--input_size', type=list, default=[416, 416]) # [416, 416] | [320, 320]
    parser.add_argument('--num_classes', type=int, default=19) # 19 | 4
    parser.add_argument('--results_path', type=str, 
                        # default='./work_dir/tmpfiles/crowdhuman_det_resluts',
                        default='./work_dir/tmpfiles/gesture_results',
                        )
    ...
    parser.add_argument('--num_candidate', type=int, default=100) # 100 | 1000
...
if __name__ == "__main__":
    ...
    kwargs_bboxeval = {
        ...
        "_COLORS": _COLORS,
        "class_names": class_names[args.model_name],
    ...
    cocoeval.set_colors(colors=None)
    cocoeval.set_class_names(names=class_names[args.model_name])
