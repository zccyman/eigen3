...
class ClsEval(Eval):
    ...
    def accuracy(self, output, gt_cls, top1_m, top5_m):
        target = gt_cls * torch.ones(1).long()
    ...
    def __call__(self, calibration_params_json_path):
        ...
        if self.fp_result:
            top1_m_float = AverageMeter()
            top5_m_float = AverageMeter()
        ...
        for image_id, (image_file, gt_cls) in enumerate(image_files):
            in_data = cv2.imread(image_file)
            if 0 == image_id:
                os.system("cp {} work_dir/".format(image_file))
            ...
            top1_m_quant, top5_m_quant = self.accuracy(
                outputs['qout'][self.output_name], 
                gt_cls, top1_m_quant, top5_m_quant,
            )
            if self.fp_result:
                top1_m_float, top5_m_float = self.accuracy(
                    outputs['true_out'][self.output_name], 
        ...
        if self.fp_result:
            accuracy['faccuracy'] = {
                "top1": top1_m_float.avg,
                "top5": top5_m_float.avg,
            }
        accuracy['qaccuracy'] = {
                "top1": top1_m_quant.avg,
                "top5": top5_m_quant.avg,
        }
