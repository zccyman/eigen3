...
cfg_file = 'benchmark/benchmark_config/pedestrian_detection.py'
args = parse_config(cfg_file)

first_model_name = ''
model_name_list = []
for model_type, model_paths in args.model_paths.items(): # type: ignore
    model_rootdir_list = []
    for model_id, model_path in enumerate(model_paths): # type: ignore
...
for quantize_method_f, quantize_method_weights in args.quantize_methods.items(): # type: ignore
    for quantize_method_w in quantize_method_weights:
        quantize_method_list.append(quantize_method_f + '/' + quantize_method_w)

process_scale_w_list = args.process_scale['weight'] # type: ignore
...
params = [str(data) for data in args.quantize_dtypes] # type: ignore
...
def _make_grid(nx=20, ny=20, i=0):
    ...
    yv, xv = torch.meshgrid(
        [torch.arange(ny).to(device),
         torch.arange(nx).to(device)])
...
def get_single_level_center_priors(batch_size, featmap_size, stride, dtype,
                                   device):
...
class Integral(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.linear(x, self.project.type_as(x)).reshape(-1, 4) # type: ignore
        return x
...
def multiclass_nms(multi_bboxes,
    ...
    dets, keep = batched_nms(bboxes, scores, labels, iou_threshold, 2000,
                             class_agnostic)
...
def get_bboxes(cls_preds,
    ...
    featmap_sizes = [(math.ceil(input_height / stride),
                      math.ceil(input_width) / stride) for stride in strides]
    ...
    bboxes = distance2bbox(center_priors[..., :2],
                           dis_preds,
...
def post_process(preds, warp_matrix, resizeed_shape, raw_shape, strides,
                 num_candidate, score_threshold, iou_threshold,
                 class_agnostic):
    cls_scores, bbox_preds = preds.split([1, 32], dim=-1)
    ...
    result_list = get_bboxes(cls_scores, bbox_preds, resizeed_shape,
                             num_candidate, strides, score_threshold,
...
class Yolov5PreProcess(object):
    ...
    def __call__(self, img):
        ...
        img_resize = letterbox(img, tuple(self.input_size),
    ...
    def __call__(self, outputs, trans):
        ...
        for i, key in enumerate(outputs.keys()):
            if outputs[key] is None:
                continue
            bs, _, ny, nx = outputs[
                key].shape  # x(bs,255,20,20) to x(bs,3,20,20,85)
            output = torch.from_numpy(outputs[key]).to(device)
            output = output.view(bs, 3, 6, ny, nx).permute(0, 1, 3, 4,
                                                           2).contiguous()
            grid[i], anchor_grid[i] = _make_grid(nx, ny, i)
            y = output.sigmoid()
            y[...,
            0:2] = (y[..., 0:2] * 2 - 0.5 + grid[i]) * self.strides[i]  # xy
        ...
        for i, det in enumerate(pred):
            det[:, :4] = scale_coords(input_size[:2], det[:, :4],
                                      raw_size[:2]).round()
            bbox.append(det[:, :4])
            score.append(det[:, -2])
            label.append(det[:, -1])
        if hasattr(torch, "cat"):
            bbox, score, label = torch.cat(bbox, dim=0), torch.cat(
                score, dim=0), torch.cat(label, dim=0)
        elif hasattr(torch, "concat"):
            bbox, score, label = torch.concat(bbox, dim=0), torch.concat(
                score, dim=0), torch.concat(label, dim=0)
        else:
            print("error version of torch!")
            os._exit(-1)
        return dict(bbox=bbox.numpy().astype(np.int32),
                    label=label.numpy().reshape(-1).astype(np.int32),
                    score=score.numpy().reshape(-1))


@BENCHMARK.register_module(name="pre2")
...
@BENCHMARK.register_module(name="post2")
class NanodetPostProcess(object):
    ...
    def __call__(self, outputs, trans):
        ...
        out = post_process(preds, ResizeM, input_shape, raw_shape,
                           self.strides, self.num_candidate,
...
@pytest.mark.usefixtures('model_paths', 'quantize_dtype', 'process_scale_w', 'quantize_method')
class TestPedestrianDetection(TestBasse):
    ...
    def compose_evaluator(self, **kwargs):
        ...
        kwargs_postprocess = {
            ...
            "strides": args.strides[net_name],
            "agnostic_nms": args.agnostic_nms,
            "num_classes": args.num_classes,
        }

        preprocess = BENCHMARK.get(preprocess_name)(**kwargs_preprocess) # type: ignore
        postprocess = BENCHMARK.get(postprocess_name)(**kwargs_postprocess) # type: ignore
        ...
        parameter = dict(quan_dataset_path=image_path,
                         ...
                         ann_path=anno_file,
                         input_size=args.input_size[imgsize],
                         normalization=args.normalizations[net_name],
    ...
    def test_pedestrian_detection(self, model_paths, quantize_dtype, process_scale_w, quantize_method, model_dir,
