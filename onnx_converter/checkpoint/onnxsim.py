...
def forward(model,
    ...
    if custom_lib is not None:
        if os.path.exists(custom_lib):
            ...
            print("No such file '{}'".format(custom_lib))
            exit(1)
...
def optimize(model: onnx.ModelProto, skip_fuse_bn: bool, skipped_optimizers: Optional[Sequence[str]]) -> onnx.ModelProto:
    ...
    for item in model.graph.initializer:
        if item.name not in test.keys():
            test[item.name] = 1
        else:
            test[item.name] = test[item.name] + 1
            items.append(item)
    for item in items:
        model.graph.initializer.remove(item)

    # for item in model.graph.initializer:
    #     if item.name in test.keys():
    #         if test[item.name] > 1:
    #             print(item.name, test[item.name])
    #             model.graph.initializer.remove(item)
    #             test[item.name] = test[item.name] - 1
...
# def remove_transpose(model: onnx.ModelProto):
#     '''
#     remove transpose after data layer and before last layer
#     '''
#
...
#         input_name = graph.inputs[i].name
#         input_names.append(input_name)
#
#     output_names = list()
#     for i in range(len(graph.outputs)):
#         output_name = graph.outputs[i].name
#         output_names.append(output_name)
#
#     fake_nodes = [node for node in graph.nodes if node.op == "Transpose"]
#
#     in_fake_nodes, out_fake_nodes = [], []
#     for fake_node in fake_nodes:
#         if fake_node.inputs[0].name in input_names:
#             in_fake_nodes.append(fake_node)
#         elif fake_node.outputs[0].name in output_names:
#             out_fake_nodes.append(fake_node)
#         else:
#             raise Exception('remove_transpose appears error !!!')
#             os._exit(-1)
#
#     for k, fake_node in enumerate(in_fake_nodes):
#         inp = fake_node.inputs[0]
#         if inp.name in input_names:
#             output_name = fake_node.outputs[0].name
#             for i, node in enumerate(graph.nodes):
#                 inputs = [n.name for n in node.inputs]
#                 if output_name in inputs:
#                     graph.nodes[i].inputs[0] = inp
#                     fake_node.inputs[0].shape = [inp.shape[i] for i in fake_node.attrs['perm']]
#                     break
#
#         fake_node.outputs.clear()
#         graph.cleanup()
#
#     for k, fake_node in enumerate(out_fake_nodes):
#         inp_node = fake_node.i()
#         inp_node.outputs = fake_node.outputs
#
#         perm = fake_node.attrs['perm']
#         for i, out in enumerate(inp_node.outputs):
#             shape_dict = dict()
#             for a, b in zip(perm, out.shape):
#                 shape_dict[a] = b
#             shape_dict = dict(sorted(shape_dict.items()))
#             inp_node.outputs[i].shape = [v for k, v in shape_dict.items()]
#             # print('test')
#
#         fake_node.outputs.clear()
#         graph.cleanup()
