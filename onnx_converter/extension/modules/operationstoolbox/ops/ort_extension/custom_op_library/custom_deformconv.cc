...
void gemm_ref_fp32_deform(const float *A,
    const float *B,
    const float *V,
    const float *H,
...
float bilinear_interpolate(const float *src,
    const int64_t src_h,
    const int64_t src_w,
    const float h,
    const float w)
...
void deformable_im2col(const float *input,
    const float *offset,
    const int64_t src_h,
    const int64_t src_w,
        ...
        for (int64_t kh = 0; kh < kernel_h; ++kh) {
            for (int64_t kw = 0; kw < kernel_w; ++kw) {
                ...
                const int data_offset_w_ptr =
                    ((2 * (kh * kernel_w + kw) + 1) * dst_h + h_col) * dst_w +
                    w_col;
                ...
                *columns_ptr =
                    bilinear_interpolate(input_ptr, src_h, src_w, ih, iw);
...
void deformable_conv_forward(const float *src,
    const float *offset,
    const float *filter,
    const int64_t batch,
    const int64_t src_c,
    ...
    for (int64_t b = 0; b < batch; ++b) {
        for (int64_t g = 0; g < group; ++g) {
            deformable_im2col(
                ...
                offset +
                    b * offset_group * 2 * kernel_h * kernel_w * dst_h * dst_w,
                src_h, src_w, kernel_h, kernel_w, pad_h, pad_w, stride_h,
                stride_w, dilation_h, dilation_w, ic_per_gp, offset_group,
                dst_h, dst_w, columns);
            float *dst_ptr =
...
MMCVDeformConvKernel::MMCVDeformConvKernel(OrtApi api, const OrtKernelInfo *info)
...
void MMCVDeformConvKernel::Compute(OrtKernelContext *context)
{
    ...
    int64_t out_height = floor((in_height + 2 * padding_height -
            ...
            stride_height +
        1);
    int64_t out_width = floor((in_width + 2 * padding_width -
    ...
    deformable_conv_forward(input_data, offset_data, filter_data, batch_size,
        in_channels, in_height, in_width, out_channels, out_height, out_width,
        group, deformable_group, in_channels, out_channels, kernel_height,
