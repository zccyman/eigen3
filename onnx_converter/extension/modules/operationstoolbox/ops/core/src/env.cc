...
namespace env {
int Env::AdviseFree(void *addr, size_t length)
{
    return 2;
}

int Env::GetCPUMaxFreq(std::vector<float> *max_freqs)
{
    return 2;
}

int Env::SchedSetAffinity(const std::vector<size_t> &cpu_ids)
{
    return 2;
}

uint32_t Env::CalculateCRC32(const unsigned char *p, uint64_t n)
{
    static const uint32_t crc32_table[256] = {0x00000000, 0x77073096,
    ...
    uint64_t i;
    for (i = 0; i < n; i++) {
        crc = crc32_table[*p++ ^ (crc & 0xff)] ^ (crc >> 8);
    }
    return (~crc);
}

bool Env::CheckArrayCRC32(const unsigned char *data, uint64_t len)
{
    if (len < CRC32SIZE) {
        return false;
    }
    const uint64_t message_size = len - CRC32SIZE;
    uint32_t saved_crc32_value =
        *reinterpret_cast<const uint32_t *>(data + message_size);
    uint32_t crc32_value = Default()->CalculateCRC32(data, message_size);
    return crc32_value == saved_crc32_value;
}
int64_t LinuxBaseEnv::NowMicros()
{
    return utils::NowMicros();
}

int LinuxBaseEnv::GetCPUMaxFreq(std::vector<float> *max_freqs)
{
    utils::MACE_CHECK_NOTNULL(max_freqs);
    int cpu_count = utils::ORTGetCPUCount();
    if (cpu_count < 0) {
        return -1;
    }
    for (int cpu_id = 0; cpu_id < cpu_count; ++cpu_id) {
        std::string cpuinfo_max_freq_sys_conf = utils::MakeString(
            "/sys/devices/system/cpu/cpu", cpu_id, "/cpufreq/cpuinfo_max_freq");
    ...
    std::cout << "CPU freq: " << utils::MakeString(*max_freqs) << std::endl;

    return 0;
}

int LinuxBaseEnv::SchedSetAffinity(const std::vector<size_t> &cpu_ids)
{
    cpu_set_t mask;
    CPU_ZERO(&mask);
...
int LinuxBaseEnv::AdviseFree(void *addr, size_t length)
{
    int page_size = sysconf(_SC_PAGESIZE);
    void *addr_aligned = reinterpret_cast<void *>(
        (reinterpret_cast<uintptr_t>(addr) + page_size - 1) & (~(page_size - 1)));
    uintptr_t delta = reinterpret_cast<uintptr_t>(addr_aligned) -
    ...
    if (length >= delta + page_size) {
        ...
        if (error != 0) {
            std::cout << "Advise free failed: " << strerror(errno) << std::endl;
            return -1;
        }
    }
    return 0;
...
int LinuxEnv::SchedSetAffinity(const std::vector<size_t> &cpu_ids)
{
    // utils::MACE_UNUSED(cpu_ids);
    return 0;
}

std::vector<std::string> LinuxEnv::GetBackTraceUnsafe(int max_steps)
{
    return utils::GetBackTraceUnsafe(max_steps);
}
